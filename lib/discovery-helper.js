...
const Sensor = require('./component/sensor');
const Switch = require('./component/switch');
const BinarySensor = require('./component/binarysensor');
...
module.exports = class Helper {
  ...
  static buildDevice(dev, values = []) {
    let device = dev;

    if (device.avty_t) {
      device.current_status = values[device.avty_t];
    }

    if (device.avty) {
      device.avty.forEach((elm) => {
        if (elm.t) {
          const value = values[elm.t];
          device.current_status = value;
          if ('pl_avail' in elm) {
            if (elm.pl_avail === value) {
              device.current_status = 'online';
            }
          } else if ('pl_not_avail' in elm) {
            if (elm.pl_not_avail === value) {
    ...
    if (device.component === 'sensor') {
      device = Sensor.build(device, values);
    }

    if (device.component === 'switch') {
      device = Switch.build(device, values);
    }

    if (device.component === 'binary_sensor') {
      device = BinarySensor.build(device, values);
    }
